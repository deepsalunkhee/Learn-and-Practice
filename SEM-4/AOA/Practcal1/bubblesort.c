...
int main(){
    ...
    Rand = fopen("E:\\GIt\\SEM-4\\AOA\\Practcal1\\arr.txt", "w"); //This will open the file in given mode
    for(i=0; i<n; i++){
        fprintf(Rand,"%d\n",(rand())); // Writes random input in the input file
    }
    fclose(Rand);
    Rand = fopen("E:\\GIt\\SEM-4\\AOA\\Practcal1\\arr.txt", "r");
    for(i=0; i<n; i++){
    ...
    //Timer Stoped
    Sort = fopen("E:\\GIt\\SEM-4\\AOA\\Practcal1\\arrsort.txt", "w");
    for(i=0; i<n; i++){
